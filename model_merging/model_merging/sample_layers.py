...
def sample_from_vector(vector, weight_vec, center, sampling_config):
    ...
    loc_probabilities = np.exp(-0.5 * ((np.arange(len(vector)) - center) / sampling_config['spread']) ** 2)
    loc_probabilities /= loc_probabilities.sum()  # Normalize to make it a probability distribution
    return np.random.choice(
        vector,
        p=sampling_config['alpha_blend'] * loc_probabilities + (1 - sampling_config['alpha_blend']) * weight_vec
    )


def generate_weighted_vector(input_vectors, weight_vectors=None, sampling_config=None):
    """
    ...
    for i in range(sampling_config['output_length']):
        ...
        result.append(rng.choice(a=[value1, value2],
...
def generate_layer_config(fishers, sampling_config=None):
    ...
    sampling_config['alpha_blend'] = sampling_config['alpha_blend'] if sampling_config is not None else 0.5
    sampling_config['beta_choice'] = sampling_config['beta_choice'] if sampling_config is not None else 0.5
    sampling_config['loc_spread'] = sampling_config['loc_spread'] if sampling_config is not None else 1.0
    sampling_config['output_length'] = sampling_config['output_length'] if sampling_config is not None else 12
    ...
    layers = generate_weighted_vector(input_vectors=layer_indices,
